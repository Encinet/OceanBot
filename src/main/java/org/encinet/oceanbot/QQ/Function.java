...
public class Function {
    public static String on(String text, Long qqNum) {
        ...
        switch (str[0]) {// 截取首位字符以后的东西
            ...
            case "whois" -> Whois.core(str.length < 2 ? null : str[1], qqNum);
            case "c" -> {
                StringBuilder c = new StringBuilder();
                if (str.length < 2) {
                    MiraiBot.getBot(Config.BotID).getGroup(Config.GroupID).sendMessage("你还没有输入命令呢");
                } else {
                    if (!hasPermission(qqNum)) {
                        rText = "没有权限";
                    } else {
                        ...
                        new BukkitRunnable() {
                            ...
                            public void run() {
                                ...
                                MiraiBot.getBot(Config.BotID).getGroup(Config.GroupID).sendMessage("指令发送完成");
                            }
