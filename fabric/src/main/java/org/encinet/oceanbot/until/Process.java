...
import org.encinet.kitebot.file.Whitelist;
...
public class Process {
    ...
    public static long stringToQBind(String text) {
        Whitelist whitelist = KiteBot.whitelist;
        try {
            if (text.startsWith("@")) {
                // @123
            ...
            // 尝试为游戏ID
            PlayerManager playerManager = KiteBot.server.getPlayerManager();
            UUID uuid = playerManager.getPlayer(text).getUUID();
            return whitelist.getBind(uuid).qq();
        }
    }
}
